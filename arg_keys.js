...
// 手动定义JSON对象，解决"JSON未定义"问题
if (typeof JSON === 'undefined') {
    JSON = {
        stringify: function(obj) {
            // 基础的JSON序列化实现（处理简单对象和数组）
            ...
            if (typeof obj === 'object') {
                ...
                for (var key in obj) {
                    if (obj.hasOwnProperty(key)) {
                        keys.push('"' + key.replace(/"/g, '\\"') + '":' + JSON.stringify(obj[key]));
                    }
                }
                return '{' + keys.join(',') + '}';
            }
        ...
        parse: function(str) {
            // 利用eval实现简单的JSON解析（仅在可信环境中使用）
            return eval('(' + str + ')');
        }
    };
}
// 补充Array.isArray()的兼容实现
if (!Array.isArray) {
...
function GetKey(i0x) {
    ...
    var bVz0x = asrsea(JSON.stringify(i0x), e, f, g);
    return bVz0x
}
